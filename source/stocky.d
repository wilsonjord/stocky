...
alias EODRecord3 = Tuple!(string,"symbol",DateTime,"time",double,"open",double,"high",double,"low",double,"close",size_t,"volumn");
...
auto onBalanceVolumn(string field="", Range)(Range rng) {
    auto myPriceRng = rng.convertRange!field;
    auto myVolumnRng = rng.convertRange!"volumn";

    double[] obv = [0];

    foreach (
        prePrice, preVolumn, curPrice, curVolumn;
        zip(myPriceRng, myVolumnRng)
            .slide!(No.withPartial)(2)
            .map!(a => tuple(a[0][0], a[0][1], a[1][0], a[1][1]))) {

        if (curPrice > prePrice) {
            obv ~= obv.back + curVolumn;
...
unittest {
    auto records = zip(
            [53.26, 53.30, 53.32, 53.72, 54.19, 53.92, 54.65],
            [0, 8_200, 8_100, 8_300, 8_900, 9_200, 13_300])
                .map!(a => tuple!("close", "volumn")(a[0], a[1]))
                .array;

    assert(records.onBalanceVolumn!"close".drop(1).equal([
                8_200, 16_300, 24_600, 33_500, 24_300, 37_600]));
}
/++
    Params:
        field = define what member to use for calculation
