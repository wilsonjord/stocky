...
+/
alias EODRecord = Tuple!(DateTime, "time", double, "open", double,
        "high", double, "low", double, "close", size_t, "volumn");
alias EODRecord2 = Tuple!(DateTime, "time", double, "open", double, "high",
        double, "low", double, "close", size_t, "volumn", double, "divReturn");
...
auto tradify(T)(T rng) {
    ...
    if (rng.count == 1)
        return rvalue;
    if (rng.count == 2 && rng.front.action != Action.buy)
        return rvalue;
...
auto createSMA(T)(T rng, int period) {
    struct SMA(Range) if (isNumeric!(ElementType!Range)) {
        ...
        auto popFront() {
            rng.popFront;
            if (!rng.empty) {
                currentResult = currentResult + (rng.front / period) - (
                        buffer.back / period);
...
auto readJson(string fileName) {
    ...
    return json["history"].object
        ...
        .map!(a => tuple!("time",
                "close")(Date.fromISOExtString(a.key), a.value["close"].str.to!double))
        .array
        .sort!((a, b) => a.time < b.time);
}

enum RecordSeriesType {
    daily,
    monthly
}
...
auto annualVolatility(string field = "", Range)(Range rng,
        RecordSeriesType type = RecordSeriesType.daily) {
    ...
    return myRng.slide!(No.withPartial)(2).map!(a => logReturn(a.front,
            a.drop(1).front)).stdev * sqrt(numPeriods.to!double);
}

unittest {
    assert([
        147.82, 149.5, 149.78, 149.86, 149.93, 150.89, 152.39,
        153.74, 152.79, 151.23, 151.78
    ].annualVolatility.approxEqual(0.110477));
}

auto volatility(T)(T records) {

    auto numRecords = 0;
    auto volatility = records.slide!(No.withPartial)(2)
        .map!(a => tuple!("time", "dailyReturn")(a[1].time, a[1].close / a[0]
                .close - 1))
        .filter!(a => a.time.year >= 2016)
        .chunkBy!((a, b) => a.time.year == b.time.year)
        .map!(a => a.map!(b => b.dailyReturn).stdev)
        .tee!(a => numRecords++)
        .enumerate(1).map!(a => a.index * a.value).sum / iota(1, numRecords + 1)
        .sum;
}
...
auto stdDeviation(string field = "", Range)(Range rng, size_t period) {
    ...
    return chain(repeat(double.nan, period - 1),
            myRng.slide!(No.withPartial)(period).map!(a => a.stdev));
}
...
auto sharpeRatio(string field = "", Range)(Range rng, size_t period) {
    ...
    return chain(repeat(double.nan, period - 1),
            myRng.dailyReturns.slide!(No.withPartial)(period).map!(a => a.mean / a
                .stdev));
}
...
auto sma(string field = "", Range)(Range rng, int period) {
    auto myRng = rng.convertRange!field;
    return chain(repeat(double.nan, period - 1),
            myRng.slide!(No.withPartial)(period).map!(a => a.mean));
...
auto returns(string field = "", Range)(Range rng, int period) {
    auto myRng = rng.convertRange!field;
    return chain(repeat(double.nan, period - 1),
            myRng.slide!(No.withPartial)(period).map!(a => a.dailyReturns.mean));
...
auto high(string field = "", Range)(Range rng, int period) {
    auto myRng = rng.convertRange!field;
    return chain(repeat(double.nan, period - 1),
            myRng.slide!(No.withPartial)(period).map!(a => a.maxElement));
}
...
auto low(string field = "", Range)(Range rng, int period) {
    auto myRng = rng.convertRange!field;
    return chain(repeat(double.nan, period - 1),
            myRng.slide!(No.withPartial)(period).map!(a => a.minElement));
}
...
unittest {
    ...
    assert(y.sma(3).filter!(a => !a.isNaN).approxEqual([4.67]));

    // using user type
    auto c = [
        EODRecord(DateTime(2000, 1, 1, 0, 0, 0), 11, // open
                15, // high
                10, // low
                13, // close
                1), // volumn
        EODRecord(DateTime(2000, 1, 2, 0, 0, 0), 12, 16,
                11, 14, 1),
        EODRecord(DateTime(2000, 1, 3, 0, 0, 0), 11, 15, 10, 15, 1)
    ];

    assert(c.sma!"close"(3).drop(2).front.approxEqual(14));

    assert(c.sma!"open"(3).drop(2).front.approxEqual((11 + 12 + 11) / 3.0));

    assert(c.map!(a => a.open).sma(3).drop(2).equal(c.sma!"open"(3).drop(2)));
...
+/
auto ema(string field = "", Range)(Range rng, int period, double seed = double
        .init) {
...
unittest {
    ...
    auto close = [
        27.62, 27.25, 26.74, 26.69, 26.55, 26.7, 26.46, 26.83, 26.89,
        27.21, 27.04, 27.25, 27.25, 27.15, 27.61, 27.63, 27.88,
        27.91, 28.01, 27.85, 27.45, 27.93, 27.44, 27.5, 27.34, 27.28,
        27.55, 27.86, 27.88, 28.03, 28.04, 28.01, 28.05, 27.87,
        27.49, 27.76, 27.37, 27.37, 27.81, 27.8, 27.95, 28.15, 28.35,
        28.09, 28.14, 28, 27.87, 27.91, 27.92, 28.14
    ];
    ...
    assert(close.ema(26).drop(25).map!(a => format("%0.5f", a))
            .equal([
                27.2869230769231, 27.3064102564103, 27.3474169040836,
                27.3868675037811, 27.4345069479455, 27.4793582851347,
                27.5186650788284, 27.5580232211374, 27.5811326121643,
                27.5743820483002, 27.5881315262039, 27.571973635374,
                27.5570126253463, 27.5757524308762, 27.5923633619224,
                27.618854964743, 27.6581990414287, 27.7094435568784,
                27.7376329230356, 27.7674378916996,
                27.7846647145367, 27.7909858467932,
                27.7998017099937, 27.8087052870312, 27.83324563614
            ].map!(a => format("%0.5f", a))));

    assert(zip(close.ema(26, 27.62).take(5), [
                27.62, 27.59259259, 27.52943759, 27.46725702,
                27.39931206
    ...
    auto b = [
        22.81, 23.09, 22.91, 23.23, 22.83, 23.05, 23.02, 23.29,
        23.41, 23.49, 24.6, 24.63, 24.51, 23.73, 23.31, 23.53, 23.06,
        23.25, 23.12, 22.8, 22.84
    ];
    assert(b.ema(9, 22.81).approxEqual([
            22.81, 22.87, 22.87, 22.95, 22.92, 22.95, 22.96, 23.03,
            23.1, 23.18, 23.47, 23.7, 23.86, 23.83, 23.73, 23.69,
            23.56, 23.5, 23.42, 23.3, 23.21
            ]));

    auto c = iota(1, 100).map!(a => tuple!("index", "value")(a, a));
    assert(c.ema!"value"(10, 1).drop(9).take(3).approxEqual([
            6.24, 7.10, 7.99
            ]));
}
...
+/
auto dema(string field = "", Range)(Range rng, int period, double seed = double
        .init) {
    auto emaRng = rng.ema!field(period, seed).array; // TODO check use of array
    return zip(emaRng, emaRng.ema!field(period)).map!(a => 2 * a[0] - a[1]);
}
...
+/
auto tema(string field = "", Range)(Range rng, int period, double seed = double
        .init) {
    auto emaRng = rng.ema!field(period, seed).array; // TODO check use of array
    return zip(emaRng, emaRng.ema!field(period),
            emaRng.ema!field(period).ema!field(period)).map!(a => 3 * a[0] - 3
            * a[1] + a[2]);
}
...
auto createK(Range)(Range rng, int windowSize)
        if (isNumeric!(ElementType!Range)) {
    return 0.to!(ElementType!Range)
        ...
        .chain(rng.retro // newest to oldest
                ...
                .map!(a => (a[0] - a.minElement).to!double / (a.maxElement - a
                    .minElement)).retro); // revert back to oldest to newest
}
...
+/
auto stochasticOscillator(string field = "", Range)(Range rng,
        int lookBack = 14, int kPeriod = 1, int dPeriod = 3) {
    auto myRng = rng.convertRange!field;

    return zip(myRng.createK(lookBack).sma(kPeriod), // the k line
            myRng.createK(lookBack).sma(kPeriod).sma(dPeriod)) // the d line
...
unittest {
    ...
    a.stochasticOscillator(3).map!(a => [a.K, a.D])
        .approxEqual([
            [0.000, 0.000], [0.000, 0.000], [0.667, 0.222],
            [1.000, 0.556], [1.000, 0.556], [1.000, 0.889],
            [0.000, 0.667], [0.429, 0.476], [0.000, 0.143],
            [1.000, 0.476], [1.000, 0.476], [0.000, 0.333],
            [0.571, 0.524], [0.750, 0.440]
        ]);

    auto b = a.map!(a => tuple!("close")(a));
    b.stochasticOscillator!"close"(3).map!(a => [a.K, a.D])
        .approxEqual([
            [0.000, 0.000], [0.000, 0.000], [0.667, 0.222],
            [1.000, 0.556], [1.000, 0.556], [1.000, 0.889],
            [0.000, 0.667], [0.429, 0.476], [0.000, 0.143],
    ...
    // moving average k
    // moving average d
    .map!(a => [a.K, a.D]).approxEqual([
        [0.000, 0.000], [0.000, 0.000], [0.222, 0.074], [0.556,
            0.259], [0.889, 0.556], [0.667, 0.704], [0.476, 0.677],
        [0.143, 0.429], [0.476, 0.365], [0.333, 0.317], [0.524, 0.444],
        [0.440, 0.433]
    ]);
}
...
auto rsi(string field = "", Range)(Range rng, int period) {
    ...
    auto lossSeed = myRng.changes.drop(nas).take(period)
        .filter!(a => a < 0).sum.abs / period;
    ...
    auto gains = chain(myRng.take(nas), double.nan.repeat(period),
            [gainSeed], myRng.changes.drop(nas).drop(period + 1).map!(a => getGain(
                a)));

    auto losses = chain(myRng.take(nas), double.nan.repeat(period),
            [lossSeed], myRng.changes.drop(nas).drop(period + 1).map!(a => getLoss(
                a)));
...
unittest {
    auto a = [
        44.34, 44.0902, 44.1497, 43.6124, 44.3278, 44.8264, 45.0955,
        45.4245, 45.8433, 46.0826, 45.8931, 46.0328, 45.614, 46.282,
        46.282, 46.0028, 46.0328, 46.4116, 46.2222, 45.6439,
        46.2122, 46.2521, 45.7137,
        46.4515, 45.7835, 45.3548, 44.0288, 44.1783, 44.2181,
        44.5672, 43.4205, 42.6628, 43.1314
    ];

    assert(a.rsi(14).drop(14).approxEqual([
            70.532789483695, 66.3185618051723, 66.5498299355276,
            69.4063053388443, 66.3551690562718, 57.9748557143082,
            62.929606754597, 63.2571475625453, 56.0592987152632,
            62.3770714431804, 54.7075730812613, 50.4227744114564,
            39.9898231453766, 41.4604819757056, 41.8689160925433,
            45.4632124452868, 37.3040420898597, 33.0795229943885,
            37.7729521144349
            ]));
...
auto changes(string field = "", Range)(Range rng) {
    auto myRng = rng.convertRange!field;
    return chain([double.nan], rng.convertRange!field.slide(2).map!(a => a[1] - a[0]));
}
...
auto results(T)(in T trades) pure {
    return trades.chunks(2).map!(a => a.result);
}

///
unittest {
    Trade[] trades = [
        Trade(DateTime(Date(2000, 1, 1)), 3, Action.buy),
    ...
    assert(trades.results.approxEqual([0.333, -0.111]));
}
...
auto daysHeld(T)(in T trades) pure {
    return trades.chunks(2).map!(a => (a[1].time - a[0].time).total!"days").sum;
}

auto daysHeld2(T)(in T trades) pure {
    return trades.chunks(2).map!(a => (a[1].time - a[0].time).total!"days");
...
auto tradesPerYear(T)(in T trades) pure {
    return (trades.count / 2).to!double / (
            ((trades.back.time - trades.front.time).total!"days") / 365.25);
...
unittest {
    ...
    assert(trades.tradesPerYear.approxEqual(3 / ((365 * 2 + 1) / 365.25)));
}
...
auto winlossRate(string type, T)(in T trades, Flag!"weighted" flag, double subtract) pure {
    ...
    if (flag == Yes.weighted) {
        return trades.results.enumerate(1)
            .map!(a => (myCompare(a.value - subtract, 0)) ? a.index : 0).sum
            / iota(1, trades.results.count + 1).sum.to!double;
    } else {
        return trades.results.count!(a => myCompare(a - subtract,
                0)) / trades.results.count.to!double;
    }
}
...
+/
auto winRate(T)(in T trades, double subtract = 0, Flag!"weighted" flag = No
        .weighted) pure {
...
unittest {
    ...
    assert(trades.winRate.approxEqual(2.0 / 4));
    ...
    // are the "oldest"
    assert(trades.winRate(Yes.weighted).approxEqual(0.3));
...
+/
auto lossRate(T)(in T trades, double subtract = 0, Flag!"weighted" flag = No
        .weighted) pure {
...
unittest {
    ...
    // 4 trades, 2 winning, 1 losing
    assert(trades.lossRate.approxEqual(1.0 / 4));

    assert(trades.lossRate(0.01).approxEqual(2.0 / 4)); // two losses now, due to
    // brokerage of 1%

    // sequentially, trades go: win, win, loss, draw
    // the loss is given more weighting than wins, as
    // it is "newer"
    assert(trades.lossRate(Yes.weighted).approxEqual(0.3));
...
unittest {
    ...
    assert(trades.wins.approxEqual([
    ...
    assert(trades.wins(0.01) // 1% commision
            .approxEqual([0.333 - 0.01, 0.1 - 0.01, 0.0889 - 0.01]));
}
...
unittest {
    ...
    assert(trades.profitFactor.approxEqual((0.174 * 0.6) / (0.067 * 0.2))); // avg wins * win rate / avg loss * loss rate

    assert(trades.profitFactor(0.01).approxEqual((0.164 * 0.6) / (0.0433 * 0.4)));
}
...
auto annualROI(T)(in T trades, double cost, double brokerage = 0) {
    ...
    auto flows = trades.chunks(2).map!(a => tradeValue(a));
...
auto profitPerDay(T)(in T trades, double cost, double brokerage = 0) {
    ...
    return trades.chunks(2).map!(a => tradeValue(a)).sum / trades.daysHeld;
}

auto weighted(T)(T profitRange, double brokerage = 0) pure {
    return profitRange.enumerate(1)
        .map!(a => a.index * (a.value - brokerage)).sum / iota(1,
                profitRange.count + 1).sum;
}
...
alias NamedTrade = Tuple!(string, "symbol", DateTime, "time", double,
        "price", Action, "action");
...
auto tradeAction(T)(T data, string ignore = "No") { // TODO investigate adding no trade to start
    ...
    // test buy
    if (firstHalf.all!(a => a[2] > a[0] - a[1]) && lastHalf.all!(a => a[2] <= a[0] - a[1])) {

        if (ignore == "Yes") {
            if (firstHalf.front[0] > firstHalf.front[1])
                return Action.buy;
        } else {
            return Action.buy;
        }
...
auto simulatedPrice(double price, double mu, double sigma,
        RecordSeriesType type = RecordSeriesType.daily) {
    ...
    return price * exp((mu - (pow(sigma,
            2) / 2)) * (1 / numPeriods.to!double) + (
