...
auto csvFromUtf8File(string filename)
{
    ...
    auto tmp = cast (string) read (filename);

    //import std.conv : to;
    //auto tmp = read(filename).to!string;
    ...
    if ((cast(ubyte[])(tmp[0..3])).getBOM.schema==BOM.utf8) {
        return csvToArray (tmp[3..$]);
    } else {
        return csvToArray (tmp);
    }
}
