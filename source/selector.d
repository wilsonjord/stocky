...
enum PickStrategy {winRate, weightedReturns, winReturns, averageReturns, meanReturns, averageWeightedFactor, sharpeWeighted, weightedReturnsAll}
...
auto moneyManagement(string file, PickStrategy ps) {
    ...
    auto getMetric(string symbol, DateTime t, PickStrategy pick) {
        ...
        if (trades.empty) return rvalue;

        if (trades.count <= 20) return rvalue;


        auto pastTrades = trades.take (trades.count!(a => a.action==Action.sell && a.time < t)*2);
        ...
        switch (pick) {
            ...
            case PickStrategy.sharpeWeighted:
                auto w = (pastTrades.wins.mean / pastTrades.wins.stdev) * pastTrades.wins.weighted;
                if (!w.isNaN) {
                    rvalue = w;
                }
                break;
    ...
    foreach (i; 0..2_000) {
        ...
        auto buySignals = signals.filter!(a => a.trade.action==Action.buy && a.trade.time==date).array;

        if (buySignals.count > 1) {
            import std.math : isNaN;
            auto x = buySignals.map!(a => tuple(a.symbol,getMetric(a.symbol,date,ps))).array;
            ...
            auto start = signals.find!(a => a.symbol==metrics.front[0] && a.trade.time==date);
            assert (start.front.trade.action==Action.buy);

            auto startPrice = start.front.trade.price;

            auto end = start.find!(a => a.symbol==metrics.front[0] && a.trade.time > date);
...
/+     trades.byKey
    ...
    trades.byKey
          ...
          .map!(a => tuple!("tradeAction","tradeMetrics")(a,trades[a.symbol].weightedReturns))
          .array
          //.filter!(a => a[1][0] >= 0.015)
          .take(80)
          .array
          .multiSort!((a,b) => a.tradeAction.trade.time > b.tradeAction.trade.time, (a,b) => a.tradeMetrics.weighted > b.tradeMetrics.weighted)
		  .array
          //.each!(a => writeln(a));
		  .each!(a => writeln (format("%s\t%s\t%s\t%s\t%s",
                                       a.tradeAction.symbol,
									   a.tradeAction.trade.time,
                                       a.tradeAction.trade.price,
                                       a.tradeAction.trade.action,
									   a.tradeMetrics.weighted)));
	                                   //a[0][1])));
